...
int main(int argc, char** argv){
	...
	int max_socket_so_far = 0, sd;
	//create fd_set for select
	fd_set current_sockets;
	//empty the current_sockes
	FD_ZERO(&current_sockets);
	//set the server socket on current sockets
	...
	for(int z = 0; z<11; z++){
		isset = FD_ISSET(z, &current_sockets);
		printf("%d:%d ",z,isset);
	}
	printf("\n");
	...
	while(1){
		...
		//select
		printf("selecting\n");
		if( (select(max_socket_so_far+1, &ready_sockets, NULL, NULL, &tv) )< 0 ){
		...
		for(int i = 0; i <= max_socket_so_far; i++){
			...
			if(FD_ISSET(i, &current_sockets)){
				...
				printf(" i = %d and server_socket = %d\n",i,server_socket);
				if (i == server_socket){
					...
					printf("Your client_socket is %d\n",client_socket );
...
void ISA_test(){
    ...
    stripe->data_block = malloc(sizeof(unsigned char *));
    stripe->parity_block = malloc(sizeof(unsigned char *));
